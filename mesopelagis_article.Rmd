...
common.habitat <- read_csv("habitat_fish_categories.csv") %>% 
...
col.pal <- redmonder.pal(4,"qMSOBu")[2:4]
...
ggsave("pprey_ratio_pelagic_plot.png", ratio.plot.meso, device="png",width=16, height = 13, units="cm")
...
ggsave("pprey_ratio_pelagic_plot_wf.png", ratio.plot.meso, device="png",width=16, height = 13, units="cm")

col.pal <- redmonder.pal(8,"qMSOBu")[2:8]
...
ratio.plot.all <- ratio.data %>% 
  ...
  filter(!guild %in% c("Mesopelagic","Detritus","Bacteria",
                       ...
                       "Filter feeders","Epibenthos","Sea turtles","Squid",
...
ratio.plot.all.wf <- ratio.data %>% 
 ...
 ggplot(aes(x=preyvalue, y=ratio, color = habitat_classification)) + 
  ...
  xlab("Prey availability value") +
  stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1, colour="grey50")   
ggsave("pprey_ratio_plot_wf.png", ratio.plot.all.wf, device="png",width=20, height = 10, dpi=300)
```
...
mycols2 <- redmonder.pal(8,"qMSOAsp")[2:8]
...
col.pal <- redmonder.pal(8,"qMSOBu")[c(4,8,2,3,7)]

ratio.data.catch.wf <- ratio.data %>% 
    filter(Box %in% west.fl.pol) %>% 
    filter(!guild %in% c("Mesopelagic","Detritus","Bacteria",
                       "Plankton", "Primary producers","Infauna",
                       "Filter feeders","Epibenthos","Sea turtles","Squid",
                       "Seabirds","Marine mammals")) %>% 
  left_join(common.habitat, by="long_name") %>% 
   group_by(preyvalue, guild, long_name, habitat_classification) %>% 
  ...
  ratio.plot.catch.wf <- ratio.data.catch.wf %>% 
...
ggsave("pprey_ratio_catch_guild_wf.png", ratio.plot.catch.wf, device="png",width=14, height = 15, units="cm")

ratio.data.catch <- ratio.data %>% 
    filter(Box %in% west.fl.pol) %>% 
    filter(!guild %in% c("Mesopelagic","Detritus","Bacteria",
                       "Plankton", "Primary producers","Infauna",
                       "Filter feeders","Epibenthos","Sea turtles","Squid",
...
ratio.plot.catch <- ratio.data.catch %>%
   ggplot(aes(x=preyvalue, y=catch_ratio, color = habitat_classification)) + 
  geom_point()+
  scale_color_manual(values=col.pal)+
  labs(color="Habitat classification")+
...
ggsave("pprey_ratio_catch_guild.png", ratio.plot.catch, device="png",width=20, height = 10, dpi=300)
...
nooil.data <- lapply(file.nums,get_catch_fishery,prey.meso.list,max.time, this.scenario="no_oil") %>% 
  bind_rows() %>% 
...
catch.data <- read_csv("NOAA_foss_commercial_landings_GOM.csv") %>% 
...
value.group <- group.composition %>% 
  left_join(catch.data, by="scientific_name") %>% 
  filter(!is.na(dollar_kg)) %>%
  group_by(group) %>% 
  summarise(dollar_kg = mean(dollar_kg)) 

group.table %>% 
  left_join(value.group, by="group") %>% 
  filter(is.na(dollar_kg))

write_csv(value.group, "catch_value_table.csv")
