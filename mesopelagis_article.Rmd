...
shape.file <- "GOM_LL.shp"
file.name <- "gom_model_map.png"
scale.factor <- 2
bar.position  <-  "tl"

make_map(shape.file, file.name, scale.factor, bar.position, west.fl.pol)
...
preynames="prey_names.csv"
groupnames="Group_Names.csv"
availabilitymatrix = "availabilities_matrix.csv"
filename="gom_foodweb.png"
network.type = "prey"
fishguilds = "fish_habitat_guild.csv"

foodweb_map <- diet_plot(preynames,groupnames,availabilitymatrix,filename,network.type,fishguilds)
...
meso.pred <- read_csv("mesopelagic_pred.csv") %>% 
  ...
  mutate(habitat = if_else(prop_end!=0,habitat_classification,NA_character_)) %>% 
  mutate(`predator name` = as.factor(`predator name`))
...
meso.foodweb <- meso.pred %>% 
  mutate(`predator name` = stri_trans_totitle(`predator name`), prey = stri_trans_totitle(prey)) %>% 
   ggplot(aes(prey, `predator name`), fill = prop_end) + 
 #  ggplot(aes(`predator name`,prey, fill = prop_end)) + 
   geom_tile(aes(fill = habitat)) + 
   #scale_fill_gradient(low = "white", high = "dodgerblue4")+
  scale_fill_manual(values=col.pal, name = "Habitat of predator") + 
   theme_classic() +
#theme(legend.position = "none") +
#   theme(axis.text.x = element_text(angle = 90), axis.text.y = element_text(angle = 90))+
  labs(y="Predator", x = "Mesopelagic prey group") 

 
 ggsave("mesopelagic_predators.png", meso.foodweb, device="png", width=20, height = 15, units="cm")
...
piscivore.code <- heatmap.prey %>% 
  filter(prey %in% fish.groups) %>% 
  filter(tot_end_mortality > 0.25) %>% 
  distinct(predator) %>% 
...
planktivore.code <- heatmap.prey %>% 
    filter(prey %in% plankton.groups) %>% 
    filter(tot_end_mortality > 0.25) %>% 
     distinct(predator) %>% 
    filter(predator %in% fish.groups) %>% 
    pull(predator)
...
norm.indicator.data <- indicator.data %>% 
  ...
  mutate(log_base_value = log(norm_base_value), log_oil_value = log(norm_oil_value))
...
plot.data.wf <- norm.indicator.data.wf %>%
 ...
 # filter(availability < 0.000011) %>%
  mutate(ind_ratio = norm_oil_value / norm_base_value) %>% 
  dplyr::select(availability,indicator, ind_ratio) %>% 
  pivot_longer(ind_ratio,names_to = "scenario")
#  mutate(scenario = if_else(scenario=="norm_oil_value","Mesopelagic forcing","No oil")) %>%
#  filter(scenario=="Mesopelagic forcing") %>% 

col.pal <- redmonder.pal(8,"qMSO12")[2:8]

ind.plot.wf <- plot.data.wf %>% 
  ggplot(aes(x=availability, y = value, colour = indicator))+
  geom_point() + 
 ...
 # ylim(0,1)+
  ...
  ggsave("indicator_meso_wf.png", ind.plot.wf, device="png", width=10, height = 7)

  
  
  plot.data <- norm.indicator.data %>%
  mutate(indicator=if_else(indicator=="Pelagic to demersal ratio","Pel:dem ratio",
                           if_else(indicator=="Piscivore to planktonic ratio","Pisc:plank ratio",
                                   if_else(indicator=="Forage fish biomass","Forage biom.",indicator)))) %>%
...
ind.plot <- plot.data %>% 
  ...
  ggsave("indicator_meso.png", ind.plot, device="png",width=10, height = 7)
...
Plot catch per fishery

```{r}
fishery.ratio.data <- read_csv("catch_fishery_results.csv")
