...
ratio.plot.meso <- ratio.data %>% 
  ...
  geom_point()+
#  facet_wrap(long_name ~., scales="free_y")+
  facet_grid(long_name ~ ., scales = "free_y")+
  theme_classic()+
  theme(legend.position = "none")+
  ylab("Catch ratio") + 
  xlab("Prey availability value") +
...
elasmobranch.groups <- c("Filter feeding sharks","Large sharks","Small sharks","Benthic feeding sharks",
                         "Blacktip shark","Skates and rays") %>% 
  tolower()

avail.values <- ratio.data %>% 
  distinct(preyvalue) %>% 
  pull(preyvalue)

lapply(avail.values, get_indicators, ratio.data,reef.groups, pelagic.groups, demersal.groups, planktivore.groups, piscivore.groups, shrimp.groups, forage.groups, elasmobranch.groups)

get_indicators <- function(this.avvalue, ratio.data,reef.groups, pelagic.groups, demersal.groups, planktivore.groups, piscivore.groups, shrimp.groups, forage.groups, elasmobranch.groups){
  
  #Reef fish catch

this.biom.data <- ratio.data %>% 
  filter(preyvalue==this.avvalue)

reef.catch <- ratio.data %>% 
